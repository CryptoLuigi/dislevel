...
def get_leadercard(data, nick:str, bg:str):
    ...
    while position != 11:
        ...
        try:
            profile_image = Editor(load_image(data[f"profile_image_{position}"])).resize((70, 70))
            ...
            background.text((20, rank_pos), "Rank :", font=Font.Redressed(size=40, variant="bold"), color="white")
            background.text((120, rank_pos), f"{rank}", font=Font.Redressed(size=50, variant="bold"), color="white")
